...
const AuthContext: React.FC = ({ children }) => {
  ...
  const RequestAccessToken = async (code) => {
    ...
    let bodyContent = JSON.stringify({
      ...
      redirect_uri: "com.jsapp6://app/login",
      client_id: "jsapp6",
    });
  ...
  const LoginWithLichess = async () => {
    ...
    WebBrowser.openAuthSessionAsync(
      `https://lichess.org/oauth?response_type=code&client_id=jsapp6&redirect_uri=com.jsapp6://app/login&code_challenge_method=S256&code_challenge=${code_challenge}&state=${authstate.current}&scope=${scopes}`,
      "jsapp6://app/game"
    );
  };
